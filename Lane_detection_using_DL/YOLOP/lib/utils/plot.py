...
def show_seg_result(img, result, index, epoch, save_dir=None, is_ll=False,palette=None,is_demo=False,is_gt=False):
    ...
    color_mask_resized = cv2.resize(color_mask, (img.shape[1], img.shape[0]), interpolation=cv2.INTER_LINEAR)
    ...
    img[color_mask_resized != 0] = img[color_mask_resized != 0] * 0.5 + color_seg_resized[color_mask_resized != 0] * 0.5
